...
def get_tokenizer(name="gpt2", sequence_length=2048):
    ...
    #CHANGED Tokenizerをrinna社用に
    elif name == "rinnna_gpt2_medium":
        tokenizer = T5Tokenizer.from_pretrained("rinna/japanese-gpt2-medium")
        # print(tokenizer.eos_token)
        
        #NOTE 
        #2とpaddingが競合してしまっている
        #
        #誤: tokenizer.pad_token_id = tokenizer.eos_token
        #正: tokenizer.pad_token = tokenizer.eos_token
        # tokenizer.padding_side = "right"
...
def get_params_for_weight_decay_optimization(module, config):
    ...
    # print_main(f"state_dict:{module.state_dict().keys()} \n")
    # print_main(f"named_parameters: {dict(module.named_parameters()).keys()}")
    # print_main("no_w:", len(no_weight_decay_params["params"]))
    # print_main("w:", len(weight_decay_params["params"]))
    # print_main("pd:", len(param_dict.keys()))
    wdp_p = weight_decay_params["params"]
    no_wdp_p = no_weight_decay_params["params"]
    # print_main("wdp_set - pd_set", set(param_dict.keys()).difference(set(weight_decay_params["params"])))
    try:
        assert len(no_weight_decay_params["params"]) + len(
            weight_decay_params["params"]
        ) == len(
            param_dict.keys()
        ), f"Number of params in both groups != total number of trainable params\n len_params = {len(wdp_p)} + {len(no_wdp_p)}, pd = {len(param_dict.keys())}\n diff: {set(dict(module.named_parameters()).keys()) - set(module.state_dict().keys())}\n"
    except AssertionError:
        if len(no_wdp_p) == 197:
            no_wdp_p.pop(196)
        pass
        # count_tensors = {k : 0 for k in param_dict.keys()}
        # for i, p in enumerate(no_wdp_p):
        #     for k, v in param_dict.items():
        #         if torch.equal(p, v):
        #             count_tensors[k] += 1
        # # print_main([(i, p) for i, p in enumerate(named_params) if p is not None])  
        # for k, v in count_tensors.items():
        #     if v == 2:
        #         print_main("-"*100)
        #         print_main(k)
        #         print_main("-"*100)
    # state_dict: {module.state_dict().keys()}\n\n named_parameters: {dict(module.named_parameters()).keys()}\n
    assert len(no_weight_decay_params["params"]) + len(
            weight_decay_params["params"]
        ) == len(
            param_dict.keys()
