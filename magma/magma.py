...
from .language_model import get_gptj, get_japanese_gpt2_small, get_japanese_gpt2_medium
...
class Magma(nn.Module):
    def __init__(self, config, device=None):
        ...
        self.config = config
        #TODO GPT-Jから、rinna社のものへ変更
        # self.lm = get_gptj().to(self.device)
        self.lm = get_japanese_gpt2_small().to(self.device)
        
        self.seq_len = self.lm.config.max_position_embeddings
    
        #TODO GPT-Jから、rinna社のものへ変更
        self.tokenizer = get_tokenizer("rinnna_gpt2_small", sequence_length=self.seq_len)
        ...
        print_main("hidden_size: ", self.lm.config.hidden_size)
        ...
        **adapter_kwargs,
        ...
        for l in range(len(self.transformer)):
            if location == "mlp":
                ...
                #CHANGED
                # mlp2 = getattr(self.transformer[l], ff_attr)
    ...
    def forward(
        ...
        #CHANGED infereceの時はcaptionいらない
        if not inference:
    ...
    def from_checkpoint(cls, config_path, checkpoint_path, device = 'cpu'):
        ...
        #CHANGED 
        #gptを日本語版に変えたので、lmの重みはロードしなくて良い
